...
const PickersSummary = ({ data, onUserInfoChange }) => {
  ...
  const { selectedSeating, selectedDate, selectedTime, selectedOccasion, selectedDiners, policyAgreement } = data;
  const handlePolicyAgreementChange = () => {
    onUserInfoChange('policyAgreement', !policyAgreement);
  };
  return (
    <div className="pt-8 relative md:left-20 ">
      <ul className="mt-2 grid grid-cols-2 gap-8">
        <li className="text-white text-2xl flex gap-x-4 items-center">
          <BsFillCalendarMinusFill size={25} /> {formatDate(selectedDate)}
        </li>
        ...
        <li className="text-white text-xl col-start-1 col-end-3 flex justify-center items-center">
          {selectedSeating.charAt(0).toUpperCase() + selectedSeating.slice(1)} Seating
        </li>
      ...
      <label
        ...
        <span className="h-6 w-6 block rounded-full border-2 border-solid border-highlightGrey relative">
          <span
            className={`radio absolute top-1/2 left-1/2 transform -translate-x-1/2 -translate-y-1/2 bg-yellow-400 rounded-full h-2 w-2 transition duration-300 ${
              policyAgreement ? 'scale-90 ' : 'scale-0'
            }`}></span>
        </span>
